...
# Start a local Dask cluster using all available threads
cluster = LocalCluster(n_workers=1, threads_per_worker=56, memory_limit='56GB')
client = Client(cluster)

# Print client info and worker summary
print("✅ Dask cluster started.")
print(client)  # Includes dashboard URL if applicable
print("Workers:")
...
for i, path in enumerate(L2_var_paths):
    ...
    ## Save File
    if 'CRFoff' in parent_dir:
        file_name = parent_dir + '/L4/d02_sunrise_VI_DASK' + str(L2_vars[i])
    else:
        file_name = parent_dir + '/L4/d02_VI_DASK' + str(L2_vars[i])
    ...
    print(f'Total time for {L2_vars[i]} using DASK \N{check mark}', step2_time-start2_time, 'seconds')
